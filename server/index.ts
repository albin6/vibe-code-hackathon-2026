...
if (!SPREADSHEET_ID || !CLIENT_EMAIL || !PRIVATE_KEY) {
  console.warn(
    "Google Sheets env vars are not fully configured. The API will return 500 until set."
  );
}
...
app.post("/sheets", async (req, res) => {
  try {
    ...
    if (!payload || !Array.isArray(payload.participants)) {
      return res
        .status(400)
        .json({ error: "Invalid payload. Expected { participants: [...] }" });
    }
    ...
    if (!SPREADSHEET_ID || !CLIENT_EMAIL || !PRIVATE_KEY) {
      return res
        .status(500)
        .json({
          error: "Server not configured with Google Sheets credentials",
        });
    }
    ...
    // Append one row per participant for clarity
